(function() {
  var R_ENV, Relay, beautify, coffee, colors, compile, error, fs, getFiles, info, minify, onDirChanged, padLeft, parser, path, runCommand, spawn, startCompile, startWatch, stdout, test, timeStamp, uglify, write, _ref;
  fs = require("fs");
  path = require("path");
  ...
  exports.grind = function(opts, callback) {
    opts.input == null && (opts.input = "src");
    opts.output == null && (opts.output = "lib");
    opts.silent && (stdout = function() {});
    opts.requested = !1;
  ...
  getFiles = function(dir, callback) {
    return Relay.serial(Relay.func(function() {
      ...
      return err != null ? this.skip() : this.next(files);
      ...
      return err != null ? this.skip() : stats.isDirectory() ? getFiles(this.local.file, this.next) : stats.isFile() ? this.next([ this.local.file ]) : this.skip();
  ...
  write = function(filename, data, callback) {
    return Relay.serial(Relay.func(function() {
      ...
      return i !== dirs.length - 1 ? fs.mkdir(this.global.filename, this.next) : fs.writeFile(this.global.filename, data, this.next);
    }), !0)).complete(callback).start();
  };
  compile = function(code, opts, filepath, callback) {
    return Relay.serial(Relay.func(function() {
      var browser, compileOpts, details, node;
      compileOpts = {};
      opts.bare && (compileOpts.bare = opts.bare);
      if (R_ENV.test(code)) {
  ...
  startCompile = function(opts) {
    Relay.serial(Relay.func(function() {
      ...
      return err != null ? error("'" + opts.output + "' does'nt exist") : getFiles(opts.input, this.next);
    }), function() {
      return opts.join != null ? Relay.serial(Relay.func(function(files) {
        this.global.details = [];
        ...
        for (i = 0, _len = tokens.length; i < _len; i++) {
          ...
          switch (token[0]) {
           case "CLASS":
            this.local.detail["class"] == null && (this.local.detail["class"] = tokens[i + 1][1]);
            break;
           case "EXTENDS":
            this.local.detail["extends"] == null && (this.local.detail["extends"] = tokens[i + 1][1]);
          }
        }
        ...
        while (i = details.length) {
          ...
          while (i--) {
            ...
            if (detail["extends"] == null) {
              details.splice(i, 1);
              tmp.push(detail);
            } else for (_i = 0, _len = sorted.length; _i < _len; _i++) {
              d = sorted[_i];
        ...
        details = sorted;
        code = "";
        if (opts.bare != null) for (_j = 0, _len2 = details.length; _j < _len2; _j++) {
          detail = details[_j];
          code += detail.code;
        } else for (_k = 0, _len3 = details.length; _k < _len3; _k++) {
          detail = details[_k];
          code += "" + detail.code + "\n";
        }
        ...
        return err != null ? this.skip() : compile(code, opts, this.local.path, this.next);
      })));
    }(), Relay.func(function() {
      info("complete compiling".cyan.bold);
      return this.next();
    })).complete(function() {
      return opts.test != null ? test(opts) : opts.command != null ? runCommand(opts) : typeof opts.callback == "function" ? opts.callback() : void 0;
    }).start();
  };
  ...
  beautify = function(code) {
    return uglify.gen_code(uglify.ast_squeeze(parser.parse(code), {
      make_seqs: !1
    }), {
      beautify: !0,
      indent_start: 0,
  ...
  test = function(opts) {
    Relay.func(function() {
      ...
      return opts.command != null ? runCommand(opts) : typeof opts.callback == "function" ? opts.callback() : void 0;
    }).start();
  };
