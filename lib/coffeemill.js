(function() {
  var R_ENV, Relay, coffee, colors, compress, error, fs, info, onDirChanged, padLeft, parser, path, spawn, startCompile, startWatch, stdout, test, timeStamp, uglify, _ref;
  ...
  exports.grind = function(input, output, test, join, bare, compress, silent, callback) {
    var opts;
    if (input == null) input = 'src';
    if (output == null) output = 'lib';
  ...
  test = function(opts) {
    Relay.serial(Relay.func(function() {
      return fs.stat(opts.test, this.next);
    }), Relay.func(function(err, stats) {
      var nodeunit;
      if (err == null) {
        ...
        nodeunit.on('exit', function(code) {
          return info("complete testing".cyan.bold);
        });
      }
      return this.next();
    })).complete(function() {
