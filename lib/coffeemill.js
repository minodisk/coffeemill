(function() {
  ...
  generateIndexDoc = function(filepaths, opts, callback) {
    var filepath, html, name, pkg, pkgs, _i, _len;
    pkgs = {};
  ...
  compile = function(code, opts, filepath, callback) {
    return Relay.serial(Relay.func(function() {
      if (opts.copyright != null) {
        this.global.copyright = coffee.compile("###\n" + fs.readFileSync(opts.copyright) + "\n###", {
          bare: true
        });
        this.global.copyright = this.global.copyright.replace(/[\r\n]{2,}$/g, "\n");
      } else {
        this.global.copyright = "";
      }
      return this.next();
    }), Relay.func(function() {
      var browser, compileOpts, detail, details, node, _i, _len;
      compileOpts = {};
      ...
      for (_i = 0, _len = details.length; _i < _len; _i++) {
        detail = details[_i];
        detail.code = "" + this.global.copyright + detail.code;
      }
    ...
    Relay.serial(Relay.func(function() {
      ...
      if (opts.join != null) {
        ...
        return Relay.each(Relay.serial(Relay.func(function(file) {
          var exec, extension, newFileName, oldFileName, root, _ref2;
          this.local.path = getFilepath(file, opts.input);
          extension = ((_ref2 = file.match(/\.[a-z]+$/)) != null ? _ref2[0] : void 0) || "";
          root = process.cwd();
          if (file.match(/\/\.[^\/]*$/)) return this.skip();
          if (!file.match(/\.coffee$/)) {
            oldFileName = path.join(root, opts.input, this.local.path + extension);
            newFileName = path.join(root, opts.output, this.local.path + extension);
            exec = require("child_process").exec;
            exec("mkdir -p $(dirname " + newFileName + ');\ncp "' + oldFileName + '" "' + newFileName + '";');
            info("copy file: " + String(path.join(opts.output, this.local.path + extension)).bold);
            return this.skip();
          }
